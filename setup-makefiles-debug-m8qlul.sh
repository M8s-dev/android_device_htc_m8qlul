...
echo "DEBUG $0 OUTDIR=$OUTDIR"
echo "DEBUG $0 MAKEFILE=$MAKEFILE"
...
LINEEND=" \\"
COUNT=`wc -l proprietary-files-m8qlul.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' proprietary-files-m8qlul.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' proprietary-files-m8qlul.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
...
LINEEND=" \\"
COUNT=`wc -l proprietary-files-qc-m8qlul.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' proprietary-files-qc-m8qlul.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' proprietary-files-qc-m8qlul.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
...
EOF
echo "DEBUG $0 OOK NOG ../../../$OUTDIR/$DEVICE-vendor.mk"
...
echo "DEBUG $0 OOK NOG ../../../$OUTDIR/BoardConfigVendor.mk"
